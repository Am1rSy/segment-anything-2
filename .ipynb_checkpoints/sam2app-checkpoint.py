...
#         predictor.reset_state(inference_state)
    ...
    frame_interval = max(1, total_frames // fps)
...
def begin_track(predictor, inference_state, io_args, input_video):
    ...
    ffmpeg.input(os.path.join(io_args['output_masked_frame_dir'], '%07d.png'), framerate=out_fps).output(io_args['output_video'], vcodec='h264_nvenc', pix_fmt='yuv420p').run()
     # zip predicted mask
    os.system(f"zip -r {io_args['tracking_result_dir']}/{video_name}_pred_mask.zip {io_args['output_mask_dir']}")
    print("done")
    return  io_args['output_video'], io_args['output_video'], io_args['zip_mask']
